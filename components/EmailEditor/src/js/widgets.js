...
class FieldWidget extends Widget {
  replaceTag(html) {
    ...
    var placeholder = thisWidget.field.placeholder
      ? thisWidget.field.placeholder
      : '';
    html = html.replace(/\[FIELD_PLACEHOLDER\]/g, placeholder);

    if (thisWidget.field.required == 1) {
      html = html.replace(/\[FIELD_REQUIRED\]/gi, 'required');
    } else {
      html = html.replace(/\[FIELD_REQUIRED\]/gi, '');
    }
